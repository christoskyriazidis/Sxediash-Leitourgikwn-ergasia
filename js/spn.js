...
function HRRN(queue) {
  ...
  while (!finish) {
    ...
    if (requestQueue.length != 0) {
      ...
      let minBurstTime = 1000;
      //se poia 8esi tou pinaka einai to process me to mikrotero burstTime
      let minProcess = -1;
      ...
      if (requestQueue.length > 1) {
        ...
        for (let i = 0; i < requestQueue.length; i++) {
          //koitaei an to burstTime tis diergasies einai mikrotero apo thn min diergasia..
          if (requestQueue[i].BurstTime < minBurstTime) {
            minBurstTime = requestQueue[i].BurstTime;
            minProcess = i;
          }
        }
        //vazoume stn "KME" tin 1 diergasia apo to requestQueue gia ektelesh
        kme = requestQueue.splice(minProcess, 1);
